...
type AuthUserQuery struct {
	...
	withProvider *AuthProviderQuery
...
func (auq *AuthUserQuery) QueryProvider() *AuthProviderQuery {
	query := (&AuthProviderClient{config: auq.config}).Query()
...
func (auq *AuthUserQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (auq *AuthUserQuery) FirstIDX(ctx context.Context) string {
...
func (auq *AuthUserQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (auq *AuthUserQuery) OnlyIDX(ctx context.Context) string {
...
func (auq *AuthUserQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (auq *AuthUserQuery) IDsX(ctx context.Context) []string {
	ids, err := auq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (auq *AuthUserQuery) Clone() *AuthUserQuery {
	...
	return &AuthUserQuery{
		...
		withProvider: auq.withProvider.Clone(),
...
func (auq *AuthUserQuery) WithProvider(opts ...func(*AuthProviderQuery)) *AuthUserQuery {
	query := (&AuthProviderClient{config: auq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	auq.withProvider = query
	return auq
}
...
//		GroupBy(authuser.FieldCreatedAt).
...
//
//	client.AuthUser.Query().
//		Select(authuser.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (auq *AuthUserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuthUser, error) {
	...
	if query := auq.withProvider; query != nil {
		if err := auq.loadProvider(ctx, query, nodes, nil,
			func(n *AuthUser, e *AuthProvider) { n.Edges.Provider = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (auq *AuthUserQuery) loadProvider(ctx context.Context, query *AuthProviderQuery, nodes []*AuthUser, init func(*AuthUser), assign func(*AuthUser, *AuthProvider)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*AuthUser)
	for i := range nodes {
	...
	query.Where(authprovider.IDIn(ids...))
...
func (auq *AuthUserQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(authuser.Table, authuser.Columns, sqlgraph.NewFieldSpec(authuser.FieldID, field.TypeString))
	_spec.From = auq.sql
	...
	if fields := auq.ctx.Fields; len(fields) > 0 {
		...
		if auq.withProvider != nil {
			_spec.Node.AddColumnOnce(authuser.FieldProviderID)
		}
	}
